...
def dict_from_payload(base64_input: str, fport: int = None):
    ...
    tst1 = len(decoded)
    tim= time.strftime('%Y-%m-%d %H:%M', time.localtime(time.time()+36000))
    Ext = decoded[6] & 0x0F
    poll_message_status=(decoded[6] & 0x40)>>6
    Connect=(decoded[6] & 0x80)>>7
    if poll_message_status == 0:
        if Ext == 9:
          TempC_DS=(decoded[0]<<24>>16 | decoded[1])/100
          Bat_status=decoded[4]>>6
          Work_mode="DS18B20 & timestamp"
          Systimestamp=(decoded[7]<<24 | decoded[8]<<16 | decoded[9]<<8 | decoded[10] )
          TempC_SHT=(decoded[2]<<24>>16 | decoded[3])/100
        ...
        if (Ext == 1):
          ...
          TempC_DS=(decoded[7]<<24>>16 | decoded[8])/100
          BatV= ((decoded[0]<<8 | decoded[1]) & 0x3FFF)/1000
          Bat_status=decoded[0]>>6
          TempC_SHT=(decoded[2]<<24>>16 | decoded[3])/100
          Hum_SHT=((decoded[4]<<8 | decoded[5])&0xFFF)/10
        ...
        if (Ext == 2):
          Ext_sensor ="Temperature Sensor"
          TempC_TMP117=(decoded[7]<<24>>16 | decoded[8])/100
          BatV= ((decoded[0]<<8 | decoded[1]) & 0x3FFF)/1000
          Bat_status=decoded[0]>>6
          TempC_SHT=(decoded[2]<<24>>16 | decoded[3])/100
          Hum_SHT=((decoded[4]<<8 | decoded[5])&0xFFF)/10
    ...
    if poll_message_status == 1:
      ...
      while i<tst1:
        ...
        if (Ext == 1) or (Ext == 9) or (Ext == 2):
      	  bb=((decoded[0+i]<<24>>16 | decoded[1+i])/100)
        if (Ext == 4):
        ...
        cc= ((decoded[2+i]<<24>>16 | decoded[3+i])/100)
