function Decoder(bytes, port) {
   ...
   if(port==0x02)
   {
    ...
    if(bytes[2]==1)
    {
	 decode.WIND_SPEED=((bytes[11]<<8 | bytes[12])/100).toFixed(2);
	 decode.wind_speed_max = (((bytes[13]<<8)|bytes[14])/100).toFixed(2);
	 decode.wind_speed_average = (((bytes[15]<<8)|bytes[16])/100).toFixed(2);
